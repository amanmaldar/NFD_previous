...
//  buf1 << interestName;
  //std::cout << "interest name: " << interestName;
  //localhopPacket = (buf1.str())[1]; // if it is 'l', then it is localhop packet

  NFD_LOG_DEBUG("FWD onIncomingInterest face=" << inFace.getId() <<
                " interest=" << interest.getName() << "localhopPacket: ");
...
{
  NFD_LOG_DEBUG("FWD onContentStoreMiss interest=" << interest.getName() << " localhopPacket: " <<  " miss counter: " << m_counters.nCsMisses);
  std::stringstream buf3;
  auto tmp1 = interest.getName();
  buf3 << tmp1; 
   
NFD_LOG_DEBUG("wowmiss" << (buf3.str())[1] );
  if ((buf3.str())[1] !='l' && (buf3.str())[16] != 'R' && (buf3.str())[1] != NULL)
...
{
  NFD_LOG_DEBUG("FWD onContentStoreHit interest=" << interest.getName() << " localhopPacket: "<< "hit counter: " << m_counters.nCsHits);
 std::stringstream buf4;
 auto tmp2 = interest.getName();
  buf4 << tmp2;
  NFD_LOG_DEBUG(" wowhitt" << (buf4.str())[1] ); 
  if ((buf4.str())[1] !='l'&& (buf4.str())[1] != NULL && (buf4.str())[16] != 'R' )
	  ++m_counters.nCsHits;
...
{
  //NFD_LOG_DEBUG("FWD onOutgoingInterest face=" << outFace.getId() << " interest=" << pitEntry->getName());
...
{
  // receive Data
//  NFD_LOG_DEBUG("FWD onIncomingData face=" << inFace.getId() << " data=" << data.getName());
...
Forwarder::onOutgoingData(const Data& data, Face& outFace)
{
  ...
  //NFD_LOG_DEBUG("FWD onOutgoingData face=" << outFace.getId() << " data=" << data.getName());
